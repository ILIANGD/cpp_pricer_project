...
template<typename T> //lets you write code that works with any data type 
class BinaryTree {
private:
...
public:
    ...
    void setDepth(int depth) {
        ...
        for (int n = 0; n <= _depth; ++n)
            _tree[n].resize(n + 1); //allocates , deallocate the vectors in tree
    }
    ...
    void setNode(int n, int i, const T& value) {
        ...
        _tree[n][i] = value; // stores the value in the tree
    }

    const T& getNode(int n, int i) const {
        if (n < 0 || n > _depth || i < 0 || i > n)
            throw std::out_of_range("Invalid node indices");
        return _tree[n][i]; // retrieves the corresponding value
    }
    ...
    void display() const {
        ...
        for (int n = 0; n <= _depth; ++n) {
            ...
            for (int i = 0; i <= n; ++i) {
                ...
                // we rint "0" for very small or negative-zero values
                if (std::fabs(value) < 1e-9)
                    std::cout << "0";
                else
                    std::cout << std::fixed << std::setprecision(2) << value; // we print the tree with a precision of 2 decimals
            }
            std::cout << "\n";

            // we print the link lines  except after the last row 
            if (n < _depth) {
                int linkIndent = (_depth - n) * (width / 2) + width/5; // on ajoute width/5 car petit decalage
                std::cout << std::setw(linkIndent) << "";

                for (int i = 0; i <= n; ++i) {
                    std::cout << std::setw(width / 2) << "/"   // left link with setw(n) giving n spaces for the next value of print
                        << std::setw(width / 2) << "\\"; // right link
                }
                std::cout << "\n";
            }
        }
